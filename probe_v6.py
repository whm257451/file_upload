...
def data_of(o):
    ...
    d=py(d)
    return d.get('res',d) if isinstance(d,dict) else None
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('images',nargs='+',type=Path); ap.add_argument('-o','--output',type=Path,default=Path('probe_results'))
    ...
    for p in a.images:
        ...
        for tile,dx,dy in tiles(im):
            rr=ocr.predict(input=tile,text_det_limit_side_len=1280,text_det_limit_type='max',text_det_thresh=0.15,text_det_box_thresh=0.25,text_det_unclip_ratio=1.6,text_rec_score_thresh=0.0)
            for r in rr:
                d=data_of(r)
                if not d: continue
                polys=d.get('rec_polys',[]) or []; texts=d.get('rec_texts',[]) or []; scores=d.get('rec_scores',[]) or []
                for i,q in enumerate(polys):
                    z=np.asarray(q,dtype=float).reshape(-1,2); z[:,0]+=dx; z[:,1]+=dy
                    box=[float(z[:,0].min()),float(z[:,1].min()),float(z[:,0].max()),float(z[:,1].max())]
        ...
        kept.sort(key=lambda z:(z['bbox'][1],z['bbox'][0]))
        out={'input':str(p),'size':[im.shape[1],im.shape[0]],'models':[DET_MODEL,REC_MODEL],'parameters':{'tile':960,'overlap':128,'det_thresh':.15,'box_thresh':.25,'unclip':1.6},'count':len(kept),'items':kept}
        (a.output/f'{p.stem}_v6_raw.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
        vis=im.copy()
        for x in kept: cv2.polylines(vis,[np.rint(np.asarray(x['polygon'])).astype(np.int32)],True,(0,255,255),2,cv2.LINE_AA)
        ok,b=cv2.imencode('.jpg',vis,[cv2.IMWRITE_JPEG_QUALITY,94]); b.tofile(str(a.output/f'{p.stem}_v6_all.jpg'))
        summary.append({'image':str(p),'count':len(kept)})
        print(p,len(kept))
